...
$(document).ready(function () {
	...
	tiny_present = false;
	if ($('.tinyCont').length || $('.tinytextarea').length) {
		tiny_present = true;
	}
...
$(document).ready(function () {
    ...
    function show_settings_popup(thisTrigger, popupContainerClass) {
	    ...
	    if ($('.edit_popup.nowOpen').find('.tinyCont').length) {
    ...
    $(document).on('click', '.radio_toggle_cont .radio_toggle_mirage', function () {
        ...
        if (this_click_btn.hasClass("checked")) {
            ...
            radio_toggle_cont2.find('.yes_state')[0].checked = false;
            radio_toggle_cont2.find('.no_state')[0].checked = true;
        } else {
            ...
            this_click_btn.parents('.edit_popup').find('.accordion_expanded_toggle').find('.radio_toggle_mirage').each(function (index) {
                if ($(this).not(".checked")) {
                    ...
                    radio_toggle_cont.find('.no_state')[0].checked = false;
                    radio_toggle_cont.find('.yes_state')[0].checked = true;
                }
            });
    ...
    $(document).on('click', '.pb-module', function () {
        ...
        $.post(ajaxurl, data, function (response) {
            ...
            reactivate_selectbox();
            check_visual_part_for_toggles();
			if (our_tiny_firstrun == true && $('body').find('.tinyCont').length) {
				run_Tiny();
			}
        });
    });
    ...
    $(document).on('click', '.add_new_accordion_section', function () {
        ...
        $.post(ajaxurl, data, function (response) {
			...
			if (our_tiny_firstrun == true && $('body').find('.tinytextarea').length) {
				run_Tiny();
			}
