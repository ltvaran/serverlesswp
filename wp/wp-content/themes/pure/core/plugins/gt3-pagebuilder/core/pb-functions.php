...
function get_selected_pf_images_for_admin($gt3_theme_pagebuilder)
{
    ...
    if (isset($gt3_theme_pagebuilder['post-formats']['images']) && is_array($gt3_theme_pagebuilder['post-formats']['images'])) {
        foreach ($gt3_theme_pagebuilder['post-formats']['images'] as $imgid => $img) {
            ...
            $attach_id = (int)$img['attach_id'];
            $img       = wp_get_attachment_image_src($attach_id, 'full');
            $img       = $img[0];

            $compile .= "<div class='img-item style_small'><div class='img-preview'><img src='" . aq_resize($img, "62", "62", true) . "' data-full-url='{$img}' data-thumb-url='" . aq_resize($img, "156", "106", true, true, true) . "' alt='' class='previmg'><div class='hover-container'></div><div class='deldel-container'></div></div><input type='hidden' name='pagebuilder[post-formats][images][" . $attach_id . "]' value='{$attach_id}'></div>";
        }
    }
    return $compile;
}
...
function show_video_preview($videourl)
{
    ...
    if ($is_youtube > 0) {
        ...
        $compile_inner = "
            <iframe width=\"395\" height=\"295\" src=\"https://www.youtube.com/embed/" . $videoid . "\" frameborder=\"0\" allowfullscreen></iframe>
        ";
    }
    ...
    if ($is_vimeo > 0) {
        ...
        $compile_inner = "
            <iframe src=\"https://player.vimeo.com/video/" . $videoid . "\" width=\"395\" height=\"295\" frameborder=\"0\" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
        ";
    }
