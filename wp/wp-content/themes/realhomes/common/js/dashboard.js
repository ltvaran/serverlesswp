...
(function ($) {
    ...
    $document.ready(function () {
        ...
        // Run this if the page is add/edit property.
        if (isDashboardPage('dashboard-submit-property')) {
            ...
            if (jQuery().validate && jQuery().ajaxSubmit) {
                ...
                // Check for reCaptcha and attach submit handler function for its validation.
                if ($('.inspiry-recaptcha-wrapper').length) {
                    let responseWrapper = $('#dashboard-content'),
                        data = {};
                        ...
                        $(form).ajaxSubmit({
                            ...
                            success: function (ajax_response, statusText, xhr, $form) {
                                ...
                                if (response.success) {
                                    ...
                                    if (!$('.dashboard-notice.error').length) {
                                        data.type = 'error';
                                        data.message = response.message;
                                        responseWrapper.prepend(dashboardNotice(data));
                                        scrollTo(responseWrapper);
                                    }
                                }
                            }
                        });
            ...
            // Show progress loader before unload
            $window.on('beforeunload', function () {
                $dashboard.find('#dashboard-content').addClass('loading');
            });
