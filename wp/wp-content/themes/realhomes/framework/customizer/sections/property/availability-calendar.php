	...
	function inspiry_availability_calendar_customizer( WP_Customize_Manager $wp_customize ) {
		...
		if ( ! empty( $property_status_terms ) && ! is_wp_error( $property_status_terms ) ) {
			...
			foreach ( $property_status_terms as $term ) {
				$property_statuses[ $term->term_id ] = $term->name;
			}
			...
			$wp_customize->add_control( new Inspiry_Multiple_Checkbox_Customize_Control( $wp_customize, 'inspiry_statuses_to_show_availability_calendar',
				array(
					'label'   => esc_html__( 'Choose Property Statuses to Show Availability Calendar', 'framework' ),
					'section' => 'inspiry_availability_calendar',
					'choices' => $property_statuses,
				)
			) );
		}
		...
		$wp_customize->add_control(
			'inspiry_property_reservations_in_rest',
			array(
				'label'       => esc_html__( 'Property Reserved Dates in REST API', 'framework' ),
				'description' => sprintf( esc_html__( 'You can learn about WordPress REST API by %s.', 'framework' ), '<a href="https://developer.wordpress.org/rest-api/" target="_blank">clicking here</a>' ),
				'type'        => 'radio',
				'section'     => 'inspiry_availability_calendar',
				'choices'     => array(
