...
if ( ! function_exists( 'realhomes_dashboard_submit_property' ) ) {
	...
	function realhomes_dashboard_submit_property() {
		global $invalid_nonce, $submitted_successfully, $updated_successfully;
				...
				if ( $property_id > 0 ) {
					...
					if ( 'add_property' == $_POST['action'] ) {
						...
						if ( ! is_user_logged_in() && inspiry_guest_submission_enabled() ) {
							/**
							 * Send Json response for guest property submission.
							 * @since 3.13.0
							 */
	...
	add_action( 'realhomes_dashboard_submit_property', 'realhomes_dashboard_submit_property' );

	if ( ! is_user_logged_in() && inspiry_guest_submission_enabled() && function_exists( 'ere_is_reCAPTCHA_configured' ) && ere_is_reCAPTCHA_configured() ) {
		/**
		 * Verifies the google recaptcha challenge.
		 *
		 * @since 3.13.0
...
if ( ! function_exists( 'realhomes_dashboard_assets' ) ) {
	...
	function realhomes_dashboard_assets() {
		...
		wp_enqueue_style(
			'dashboard-styles',
			get_theme_file_uri( 'common/css/dashboard.css' ),
			array(),
			INSPIRY_THEME_VERSION,
			'all'
		);
...
if ( ! function_exists( 'realhomes_dashboard_dynamic_css' ) ) {
	...
	add_action( 'realhomes_dashboard_custom_css', 'realhomes_dashboard_dynamic_css' );
}
