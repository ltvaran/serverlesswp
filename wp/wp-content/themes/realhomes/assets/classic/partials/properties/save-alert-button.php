...
if ( inspiry_is_save_search_enabled() && ! empty( $_SERVER['QUERY_STRING'] ) ) {
	...
	$save_search_tooltip = get_option( 'realhomes_save_search_btn_tooltip', esc_html__( 'Receive email notification for the latest properties matching your search criteria.', 'framework' ) );
		...
		<div id="rh_save_search">
			...
			if ( is_user_logged_in() ) {
				...
				<form action="post">
					...
					<button id="rh_save_search_btn" class="real-btn btn" data-saved-label="<?php echo esc_attr( $search_saved_label ); ?>" <?php echo ( ! empty( $save_search_tooltip ) ) ? 'data-tooltip="' . esc_attr( $save_search_tooltip ) . '"' : ''; ?>>
				...
				<form action="post">
					...
					<button id="rh_save_search_btn" class="real-btn btn require-login" data-saved-label="<?php echo esc_attr( $search_saved_label ); ?>" data-login="<?php echo esc_url( $theme_login_url ); ?>" <?php echo ( ! empty( $save_search_tooltip ) ) ? 'data-tooltip="' . esc_attr( $save_search_tooltip ) . '"' : ''; ?>>
						<i class="far fa-bell"></i><?php echo esc_html( $save_search_label ); ?>
					</button>
